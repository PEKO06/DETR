...
class PositionEmbeddingSine():
    ...
    def get_position_embbed(self,mask):
        ...
        shape = []
        for i in range(3):
            shape.append([tf.shape(pos_x)[i]]+ [-1]) 
        print(shape)
        pos_x = tf.reshape(pos_x, shape)
        pos_y = tf.reshape(pos_y, shape)

        pos_emb = tf.concat([pos_y, pos_x], axis=3)
        #pos_emb = tf.reshape(pos_emb,[-1,pos_emb.shape[1]*pos_emb.shape[2],pos_emb.shape[3]])
        return pos_emb


masks = tf.placeholder(shape=[None,8,8],dtype=tf.bool)
pos_encoding = PositionEmbeddingSine(num_pos_features=128)
print(pos_encoding.get_position_embbed(masks))
'''
masks = tf.placeholder(shape=[None,8,8],dtype=tf.bool)
pos_encoding = PositionEmbeddingSine(num_pos_features=128)
print(pos_encoding.get_position_embbed(masks))
